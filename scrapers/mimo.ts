...
// 1 MiMo credit ≈ 1 token (approximately)
...
export async function scrape(): Promise<ProviderData> {
  ...
  return {
    ...
    plans: KNOWN.map((def) => {
      ...
      const tokensMonthly = credits * 1_000_000 * TOKENS_PER_CREDIT;
      return {
        name: def.name,
        category: "model_provider" as const,
        price_usd_monthly: price,
        price_usd_annual: null,
        interactions_monthly: Math.round(tokensMonthly / 4000),
        interactions_note: `${credits}M credits/mo ÷ 4k per interaction`,
